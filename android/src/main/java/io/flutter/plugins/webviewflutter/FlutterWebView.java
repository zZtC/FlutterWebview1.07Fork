...
public class FlutterWebView implements PlatformView, MethodCallHandler {
  ...
  private void loadUrl(MethodCall methodCall, Result result) {
    ...
    if(url.startsWith("file://")){
      url = "file:///android_asset/flutter_assets" + url.substring(7);
    }
    Log.d("=====loadUrl=====", url);
    loadUrlWrap(url, headers);
    result.success(null);
  }

  private void loadUrlWrap(String url){
    loadUrlWrap(url, null);
  }

  private void loadUrlWrap(String url, Map<String, String> headers){
    if(url.startsWith("assets://")){
      url = "file:///android_asset/flutter_assets/" + url.substring(9);
    }
    Log.d("=====loadUrl=====", url);
    if(headers != null){
