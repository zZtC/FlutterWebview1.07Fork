...
- (bool)loadUrl:(NSString*)url withHeaders:(NSDictionary<NSString*, NSString*>*)headers {
  ...
  if([url hasPrefix:@"assets://"]){
    ...
    if(range.location != NSNotFound){
      ...
      paramStr = [subUrl substringFromIndex:range.location];
    }
    
    // 从assets获取真实链接
    NSString* key = [FlutterDartProject lookupKeyForAsset:subUrl];
    NSURL* nsUrl = [[NSBundle mainBundle] URLForResource:key withExtension:nil];
    
    // 是否需要参数拼接
    if(paramStr != nil){
      nsUrl = [NSURL URLWithString:[nsUrl.absoluteString stringByAppendingString:paramStr]];
    }
    //创建Components
//    NSURLComponents *urlComponents = [NSURLComponents componentsWithURL:nsUrl resolvingAgainstBaseURL:NO];
//    //创建参数数组
//    NSArray *itemsArr = [NSArray arrayWithObjects:[NSURLQueryItem queryItemWithName:@"language" value:@"zh"],  nil];
//    //设置参数
